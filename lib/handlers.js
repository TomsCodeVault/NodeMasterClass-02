...
// Required data: phone
// Optional data: deliveryDate (deliveries can be scheduled out as far as 48 in advance)
...
// initialize data
  ...
  var deliveryDate = helpers.validDeliveryDate(data.payload.date,48); // Date as far as 48 hours in advance
  // Check for required data
  if(phone){
              ...
              _data.read('carts',phone,function(err,cartContents){
                if(!err && cartContents){
                  if(cartContents.items.length > 0 && cartContents.totalPrice > 0){
                    ...
                    console.log(newOrderId,orderData.items,orderData.totalPrice);
                    callback(false);
                  } else {
                    callback(400,{'Error' : 'The cart is empty or the price is 0'});
                  }
                } else {
                  callback(400,{'Error': 'Could not get contents of the specified cart'});
                }
              });
              // Make sure there are items in the cart and a total price > 0

              // Get the necessary data from the user record
              // We'll need an address and email address and name for receipt

              // Process payment
              // We'll need cc info and Stripe.com test key

              // Create order
              // Order will need to list all items from cart, user informaton, and payment information
              // User may get deleted in the future but we will keep the order data

              // Empty cart after order is successfully created

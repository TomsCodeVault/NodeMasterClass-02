...
helpers.validDeliveryDate = function(date,hours){
  if(date){
    if(!hours){
      hours = 48; // default to 48 hours
    }
    var requestedDeliveryTime = Date.parse(date);
    // make sure date is a valid date
    if(!isNaN(requestedDeliveryTime)){
      var allowableHours = hours * 60 * 60 * 1000 // milliseconds
      ...
      if(deliveryTimeFromNow >= thirtyMinutes && deliveryTimeFromNow <= allowableHours){
        ...
        var h = dateObject.getHours() < 10 ? '0'+dateObject.getHours() : dateObject.getHours();
